  ...
  button.onchange = (evt) => {
    ...
    reader.onload = function (e) {
      BABYLON.SceneLoader.Append("", reader.result, scene, function (scene) {
        if (scene.activeCamera.alpha < 0) scene.activeCamera.alpha *= -1;
        var maxY = 0;
        ...
        for (i = 0; i < meshArray.length; i++) meshArray[i].setEnabled(false);
        ...
        axios
          .post("http://localhost:4000/models/add", obj)
        ...
        if (meshArray.length - 2 >= 0)
          document.getElementById(
            "texture-select-" + String(meshArray.length - 2)
          ).style.display = "none";
        //camera.setTarget(scene.meshes[length]);
        var textureSelect = document.getElementById(
          "texture-select-" + String(meshArray.length - 1)
        );
        if (!textureSelect) {
          ...
          textureSelect.className =
            "rounded-lg bg-gray-600 outline-none cursor-pointer px-2 text-center";

          document.getElementById("select-part").appendChild(textureSelect);
          for (i = 0; i < textureLength[meshArray.length - 1]; i++) {
            var textureOption = document.getElementById(
              "texture-option-" + String(meshArray.length - 1) + "-" + String(i)
            );
            if (!textureOption) {
              ...
              textureOption.id =
                "texture-option-" +
                String(meshArray.length - 1) +
                "-" +
                String(i);
              console.log(
                scene.textures[
                  scene.textures.length -
                    textureLength[meshArray.length - 1] +
                    i
                ].name
              );
              textureOption.value = String(
                scene.textures.length - textureLength[meshArray.length - 1] + i
              );
              textureOption.text =
                scene.textures[
                  scene.textures.length -
    ...
    var saveProjectButton = document.getElementById("save-btn");
    if (!saveProjectButton) {
      saveProjectButton = document.createElement("button");
      saveProjectButton.className =
        "flex justify-center items-center px-4 py-2 bg-green-700 shadow-3xl rounded-full text-md hover:bg-green-600 text-white font-semibold cursor-pointer";
      saveProjectButton.innerHTML = "Save the Project";
      saveProjectButton.id = "save-btn";
      document
        .getElementById("save-project-div")
        .appendChild(saveProjectButton);
    }
    ...
    newModelButton.className =
      "px-4 py-2 bg-gray-600 shadow-3xl rounded-full hover:bg-gray-500 text-white font-semibold";
    ...
    if (evt.pickInfo.hit && evt.pickInfo.pickedMesh && evt.event.button === 0) {
      ...
      colorPicker.oninput = (e) => {
        if (selected.diffuseColor)
          selected.diffuseColor = new BABYLON.Color3.FromHexString(
            e.target.value
          );
        else
          selected.albedoColor = new BABYLON.Color3.FromHexString(
            e.target.value
          );
      };
    }
